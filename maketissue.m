...
myname      = 'skinvessel4';% name for files: myname_T.bin, myname_H.mci  
dirname     = sprintf('~/Documents/GitHub/sims/%s',myname);
mkdir(dirname)

time_min    = 180;    	% time duration of the simulation [min] <----- run time -----
...
mcflag      = 3;     	% launch: 0 = uniform beam, 1 = Gaussian, 2 = isotropic pt. 
...
xfocus      = Nbins/2*binsize;        % set x,position of focus
yfocus      = xfocus;        % set y,position of focus
zfocus      = inf;    	% set z,position of focus (=inf for collimated beam)

% only used if mcflag == 0 or 1 or 3 (not 2=isotropic pt.)
radius      = xfocus;   % 1/e radius of beam at tissue surface
waist       = xfocus;  	% 1/e radius of beam at focus
...
if SAVEON
    ...
    filename = sprintf('../sims/%s/%s_H.mci',myname,myname);
    ...
    filename = sprintf('../sims/%s/%s_T.bin',myname,myname);
